...
import { isPreferTsNode } from '../env';
 ...
 *
 * If the user is invoking SK via a script which is being invoked via tsx, then we
...
const isInvokedByTsx: boolean = (() => {
  ...
  for (const [i, arg] of (process.execArgv ?? []).entries()) {
...
export function registerTsProject(tsConfigPath: string): () => void {
  ...
  ].filter((v): v is (...args: unknown[]) => unknown => v !== undefined);
 ...
 */
function filterRecognizedTsConfigTsNodeOptions(jsonObject: object): {
...
export function getTranspiler(compilerOptions: CompilerOptions) {
  ...
  // Just return if transpiler was already registeredTranspilers before.
  const registrationKey = JSON.stringify(compilerOptions);
  const registrationEntry = registeredTranspilers.get(registrationKey);
  if (registeredTranspilers.has(registrationKey)) {
    if (registrationEntry?.refCount) registrationEntry.refCount++;
      ...
      cleanup: () => {
        return () => {
          ...
          if (currRegistrationEntry.refCount === 0) {
            registeredTranspilers.delete(registrationKey);
            transpilerCleanup();
          }
        };
      },
    };
    registeredTranspilers.set(registrationKey, currRegistrationEntry);
...
function readCompilerOptions(tsConfigPath: string): CompilerOptions {
  return swcNodeInstalled && !isPreferTsNode()
    ? readCompilerOptionsWithSwc(tsConfigPath)
    : readCompilerOptionsWithTypescript(tsConfigPath);
}

function readCompilerOptionsWithSwc(tsConfigPath: string) {
  const {
    readDefaultTsConfig,
...
function warnNoTranspiler() {
  logger.warn(
    `
      Unable to locate swc-node or ts-node. SK will be unable to run local ts files without transpiling.
        - To fix this, ensure @swc-node/register and @swc/core have been installed
    `,
  );
}
...
export function getTsNodeCompilerOptions(compilerOptions: CompilerOptions) {
  ...
  const result: { [key in keyof CompilerOptions]: unknown } = {
    ...compilerOptions,
  };

  for (const flag in flagMap) {
    const compilerOption = compilerOptions[flag];
    if (compilerOption) {
      const flagMapElement = flagMap[flag];
      if (flagMapElement) {
        const t = ts[flagMapElement];
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-expect-error
        result[flag] = t[compilerOption];
      }
    }
  }
  ...
  // lib option is in the format `lib.es2022.d.ts`,
  // so we need to remove the leading `lib.` and trailing `.d.ts` to make it valid
  result.lib = (result.lib as string[])?.map((value) => {
    return value.replace(/^lib\./, '').replace(/\.d\.ts$/, '');
  });

  if (result.moduleResolution && typeof result.moduleResolution === 'string') {
 ...
 */
type RemoveIndex<T> = {
  [K in keyof T as object extends Record<K, 1> ? never : K]: T[K];
};
