...
export class FsTree implements Tree {
  ...
  read(
    ...
    try {
      const content: Buffer | undefined = this.recordedChanges[
        this.rp(filePath)
      ]
        ? this.recordedChanges[this.rp(filePath)].content
        : this.fsReadFile(filePath);
  ...
  delete(filePath: string): void {
    ...
    if (this.filesForDir(this.rp(filePath)).length > 0) {
      for (const f of this.filesForDir(this.rp(filePath)))
        this.recordedChanges[f] = { isDeleted: true };
    }
    this.recordedChanges[this.rp(filePath)] = {
      isDeleted: true,
    };
    ...
    if (
      filePath !== '' &&
      this.children(dirname(this.rp(filePath))).length === 0
    ) {
  ...
  isFile(filePath: string): boolean {
    filePath = this.normalize(filePath);
    try {
      return this.recordedChanges[this.rp(filePath)]
        ? !this.recordedChanges[this.rp(filePath)].isDeleted
        : this.fsIsFile(filePath);
    } catch {
      return false;
    }
  }
  ...
  listChanges(): FileChange[] {
    ...
    for (const f of Object.keys(this.recordedChanges)) {
      if (this.recordedChanges[f].isDeleted) {
        if (this.fsExists(f)) {
          res.push({ path: f, type: 'DELETE' });
        }
      } else {
        if (this.fsExists(f)) {
          res.push({
  ...
  private assertUnlocked() {
    if (this.locked) {
      logger.error(
        `File changes have already been written to disk. Further changes were attempted`,
      );
  ...
  private directChildrenOfDir(path: string): string[] {
    ...
    for (const f of Object.keys(this.recordedChanges)) {
      if (f.startsWith(`${path}/`)) {
        ...
        const file = f.slice(Math.max(0, path.length + 1));
  ...
  private rp(pp: string): string {
    return pp.startsWith('/') ? pp.slice(1) : pp;
  }
}
...
export function printChanges(
  ...
  for (const f of fileChanges) {
    switch (f.type) {
      case 'CREATE': {
        logger.log(`${indent}${chalk.green('CREATE')} ${f.path}`);

        break;
      }
      case 'UPDATE': {
        logger.log(`${indent}${chalk.white('UPDATE')} ${f.path}`);

        break;
      }
      case 'DELETE': {
        logger.log(`${indent}${chalk.yellow('DELETE')} ${f.path}`);
