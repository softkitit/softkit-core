...
describe('auth e2e test', () => {
  ...
  describe('saml auth', () => {
    ...
    let tenantWithConfiguration: Tenant;
    let tenantWithoutConfiguration: Tenant;
    ...
    beforeEach(async () => {
      ...
      const tenants = await tenantService.findAll(undefined, 0, 2, {
        createdAt: 'asc',
      });
      ...
      samlConfiguration = await samlConfigurationService.createOrUpdateEntity({
        tenantId: tenantWithConfiguration.id,
        enabled: true,
    ...
    it('should return a 401 error if there is no configuration for tenant', async () => {
      const samlLoginResponse = await app.inject({
        ...
        headers: {
          host: `${tenantWithConfiguration.tenantUrl}`,
        },
      });
    ...
    it('try saml auth for not configured tenant', async () => {
      const samlLoginResponse = await app.inject({
        method: 'GET',
        url: `api/platform/v1/auth/sso/saml/login?tenantId=${tenantWithoutConfiguration.id}&redirectUrl=${tenantWithoutConfiguration.tenantUrl}`,
        headers: {
          host: `${tenantWithoutConfiguration.tenantUrl}`,
        },
      });
