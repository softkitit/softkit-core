...
By default `@softkit/i18n` uses the [`I18nJsonLoader`](/api/i18n/src/classes/I18nJsonLoader/) loader class. This loader reads translations from `json` files. Create a folder named `i18n` in the `src` folder of your project.
...
`@softkit/i18n` now comes with type safety as well! Click [**here**](https://@softkit/i18n.com/guides/type-safety) to see how 🎉.
...
A working example is available [here](https://github.com/softkitit/softkit-core/tree/main/libs/i18n/samples/simple).
